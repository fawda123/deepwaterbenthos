...
```{r explore, fig.height = 9, fig.width = 7}
...
```{r dendro, fig.height = 7, fig.width = 7}
...
```{r spatial, fig.width = 7, fig.height = 4}
...
```{r cca, fig.width = 8, fig.height = 8, eval = T}
# abundance data in a matrix
datbio <- biogen %>% 
  select(SampleID, nm, Abun) %>% 
  group_by(nm) %>% 
...
```{r abund, fig.height = 7, fig.width = 5}
...
```{r envgrp, fig.height = 5, fig.width = 5, out.width = '50%'}
# group data as data frame
grpdat <- grps %>% 
  data.frame(grps = .) %>% 
  rownames_to_column('SampleID')
...
```{r ternary, fig.height = 5, fig.width = 5, out.width = '50%'}
# group data as data frame
grpdat <- grps %>% 
  data.frame(grps = .) %>% 
  rownames_to_column('SampleID')
